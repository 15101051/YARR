...
def _extract_obs(obs: Observation, channels_last: bool, observation_config):
    ...
    if not channels_last:
        ...
        obs_dict = {k: np.transpose(v, [2, 0, 1]) if v.ndim == 3 else np.expand_dims(v, 0) for k, v in obs_dict.items()}
    else:
    ...
    for config, name in [
        ...
        if config.point_cloud:
            obs_dict[f'{name}_camera_extrinsics'] = obs.misc[f'{name}_camera_extrinsics']
            obs_dict[f'{name}_camera_intrinsics'] = obs.misc[f'{name}_camera_intrinsics']
    return obs_dict
